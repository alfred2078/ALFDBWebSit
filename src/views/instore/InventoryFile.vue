<template>
  <div>
        ...
        <el-row>
          <el-col :span="5">
            <el-form-item label-width="" label="">
              <el-input
                v-model="checkFrom.Erpvoucherno"
                placeholder="盘点单号"
                :disabled="true"
              ></el-input>
            </el-form-item>
        ...
        <el-row>
          ...
          <el-col :span="5">
            <el-form-item label-width="0">
              <el-button icon="el-icon-search" @click="getInfo" type="primary"
                >查询</el-button
              >
            </el-form-item>
          </el-col>
    ...
    <el-card body-style="padding:2px;">
      <el-table
        :data="GetCheckArea"
        @selection-change="GetCheckAreaChange"
        border
        :row-style="{ height: '30' }"
    ...
    >
      <el-card body-style="padding:2px;">
        <el-input
          placeholder="盘点分类"
          v-model="Materialmodel.Checktypename"
          style="width: 200px"
        ></el-input>
        <el-button
          icon="el-icon-search"
          @click="getGroupTMaterial"
          type="primary"
          >查询</el-button
        >
      </el-card>
      <el-card body-style="padding:2px;">
        ...
        >
          <el-table-column type="selection" width="55"> </el-table-column>
          <el-table-column prop="Checktypename" label="盘点分类">
          </el-table-column>
        </el-table>
      </el-card>
...
export default {
  ...
  methods: {
    ...
    getTCHECKERPVOUCHERNO() {
      ...
      getTCHECKERPVOUCHERNO().then(res => {
        ...
        if (res.Result === 1) {
          ...
          if (newTime == oldTime) {
            erpSub = min.StingAdd(
              res.Data.Erpvoucherno.substr(res.Data.Erpvoucherno.length - 5)
            );
          } else {
            erpSub = "00001";
          }
    ...
    getInfo() {
      ...
      if(min.TStock.Towarehouseno!=""){

      
        let indexs = Warehouse.indexOf(min.TStock.Towarehouseno + "");
        if (indexs < 0) {
          this.$message({
            message: "您没有此仓库权限！",
            type: "warning"
      ...
      if (min.TStock.Towarehouseno == "") {
        min.pageModel.TStock = modelListWarehouse
          .map(t => t.Warehouseno)
          .join(",");
      }
    ...
    save() {
      ...
      if (
        min.checkFrom.Erpvoucherno == "" ||
        min.checkFrom.Erpvoucherno == null
      ) {
        min.$message({
