  ...
  <div>
    ...
    <el-card body-style="padding:2px;">
      <el-table :data="Data" :row-style="{height:'30'}" :cell-style="{padding:'2px'}" :header-row-style="{height:'30'}"
        ...
        <el-table-column prop="Materialno" label="物料编码" width="120" :show-overflow-tooltip="true">
        </el-table-column>
        <el-table-column prop="Materialdesc" label="物料名称" width="150" :show-overflow-tooltip="true">
        </el-table-column>
        <el-table-column prop="Spec" label="规格型号" width="150" :show-overflow-tooltip="true">
        </el-table-column>
        <el-table-column prop="Unit" label="库存单位" width="120" >
        </el-table-column>
        <el-table-column prop="Packqty" label="包装数量" width="120" >
        </el-table-column>
        <el-table-column prop="Maintypecode" label="产品分类码" width="120" :show-overflow-tooltip="true">
        </el-table-column>
        <el-table-column prop="Maintypename" label="产品分类名称" width="150" :show-overflow-tooltip="true">
        </el-table-column>
        <el-table-column prop="Qualityday" label="保质期(天)" width="120" >
        </el-table-column>
        <el-table-column prop="Qualitymon" label="保质期(月)" width="120" >
        </el-table-column>
        <el-table-column prop="ImpDay" label="临期天数" width="120" >
        </el-table-column>
        <el-table-column prop="Checktypecode" label="盘点分类" width="120" :show-overflow-tooltip="true">
        </el-table-column>
        <el-table-column prop="Checktypename" label="盘点分类名称" width="150" :show-overflow-tooltip="true">
        </el-table-column>
        <el-table-column prop="Createtime" label="创建日期" width="150" :show-overflow-tooltip="true"> </el-table-column>
    ...
    <el-dialog title="物料管理---编辑" :show-close="true" :visible.sync="outerVisible" width="65%" :close-on-click-modal="false" :before-close="handleClose">  
     ...
     <div :style="{          
        ...
        <el-table :data="materialPackData" border :row-style="{ height: '30' }" :cell-style="{ padding: '2px' }"
          ...
          <el-table-column prop="Materialdesc" label="物料名称" width="120" :show-overflow-tooltip="true"> </el-table-column>

          <el-table-column label="外69码" width="150" >
            <template slot-scope="scope">
              <el-input v-show="doubleclick==scope.$index+1"  placeholder="请输入内容"
                v-model.number="scope.row.OuterWatercode" size="mini" width="150"  clearable>
              </el-input>
          ...
          <el-table-column label="69码" width="150" >
            <template slot-scope="scope">
              <el-input v-show="doubleclick==scope.$index+1"  placeholder="请输入内容"
                v-model.number="scope.row.Watercode" size="mini" width="150"  clearable>
              </el-input>
          ...
          <template slot-scope="scope">
            ...
            <el-popconfirm
             title="确定删除吗" @onConfirm="deleteClick(scope.row)"
            >
  ...
  export default {
    ...
    methods:{
      ...
      AddRow()
      {
        debugger;
        var min =this;
        if(min.materialPackData.length!=0&& min.materialPackData[min.materialPackData.length-1].Watercode=='')
        {
          min.$message({
                message: '请输入上一行的69码',
                type: 'warning'
      ...
      SaveMaterialPack()
      {
        ...
        min.addList.forEach(t=>{
          ...
          t.Createtime = aData;
          t.Watercode = t.Watercode==null||t.Watercode==''?null:t.Watercode+'';
          t.OuterWatercode = t.OuterWatercode=null||t.OuterWatercode==''?null:t.OuterWatercode+'';
        ...
        for (var i = 0; i < min.materialPackData.length; i++) {
            for (var j = i + 1; j < min.materialPackData.length; j++) {
                if ( min.materialPackData[i].Watercode==min.materialPackData[j].Watercode) { find = true; break;}
            }
            if (find) break;
        }
        if(find)
